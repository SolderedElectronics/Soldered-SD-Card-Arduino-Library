...
#define SDHC_BLKATTR_BLKCNT(n)                                                                                         \
    MAKE_REG_SET(n, 0x1FFF, 16) //(uint32_t)(((n) & 0x1FFF)<<16) // Blocks Count For Current Transfer
...
#define SDHC_PROCTL_WECINS                                                                                             \
    MAKE_REG_MASK(0x1, 25) //((uint32_t)0x02000000)    // Wakeup Event Enable On SD Card Insertion
...
#define SDHC_PROCTL_DTW(n)                                                                                             \
    MAKE_REG_SET(n, 0x3, 1) //(uint32_t)(((n) & 0x3)<<1)  // Data Transfer Width, 0=1bit, 1=4bit, 2=8bit
...
#define SDHC_IRQSIGEN_CEBEIEN                                                                                          \
    MAKE_REG_MASK(0x1, 18) //((uint32_t)0x00040000)    // Command End Bit Error Interrupt Enable
...
#define SDHC_IRQSIGEN_CTOEIEN                                                                                          \
    MAKE_REG_MASK(0x1, 16) //((uint32_t)0x00010000)    // Command Timeout Error Interrupt Enable
...
#define SDHC_AC12ERR_CNIBAC12E                                                                                         \
    MAKE_REG_MASK(0x1, 7)                          //((uint32_t)0x00000080)    // Command Not Issued By Auto CMD12 Error
...
#define SDHC_FEVT_CNIBAC12E                                                                                            \
    MAKE_REG_MASK(0x1, 7) //((uint32_t)0x00000080)    // Force Event Command Not Executed By Auto Command 12 Error
...
#define SDHC_FEVT_AC12TOE                                                                                              \
    MAKE_REG_MASK(0x1, 1)                      //((uint32_t)0x00000002)    // Force Event Auto Command 12 Time Out Error
...
#define SDHC_MMCBOOT_BOOTBLKCNT(n)                                                                                     \
    MAKE_REG_MASK(0xFF, 16) //(uint32_t)(((n) & 0xFFF)<<16) // stop at block gap value of automatic mode
#define SDHC_MMCBOOT_AUTOSABGEN                                                                                        \
    MAKE_REG_MASK(0x1, 7) //((uint32_t)0x00000080)    // enable auto stop at block gap function
